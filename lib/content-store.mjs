...
export function ensureAdmin(req) {
  ...
  if (providedPassword !== configuredPassword) {
    throw new Error("Невалидна админ парола.");
  }
}
...
export function createNewsItem(body) {
  ...
  const tag = String(body.tag || "Новина").trim() || "Новина";
  const bodyText = String(body.body || "").trim();
  const pinned = Boolean(body.pinned);

  if (!title || !bodyText) {
    throw new Error("Новината трябва да има заглавие и съдържание.");
  }
...
export async function listApplications(mode, newApplication) {
  const applications = await readCollection(APPLICATIONS_FILE, APPLICATIONS_PATH);
...
export async function updateApplicationStatus(body) {
  ...
  if (!id || !["pending", "reviewed", "accepted", "rejected"].includes(status)) {
    throw new Error("Невалидна промяна на кандидатура.");
  }
...
export async function listNews() {
  const news = await readCollection(NEWS_FILE, NEWS_PATH);
...
async function readCollection(localPath, blobPath) {
