  ...
  try {
    ...
    if (req.method === "POST" && url.pathname === "/api/create-checkout-session") {
      if (!STRIPE_SECRET_KEY) {
        json(res, 500, { error: "Stripe secret key is missing. Add STRIPE_SECRET_KEY to .env." });
        return;
      }
    ...
    if (req.method === "POST" && url.pathname === "/api/stripe-webhook") {
      if (!STRIPE_WEBHOOK_SECRET) {
        json(res, 500, { error: "Stripe webhook secret is missing. Add STRIPE_WEBHOOK_SECRET to .env." });
        return;
      }
...
async function handleStripeEvent(event) {
  ...
  if (!session?.id) {
    throw new Error("Stripe event is missing a checkout session.");
  }
